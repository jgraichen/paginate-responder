  ...
  module PaginateResponder
    def to_format
      if get? && resource.respond_to?(:paginate)
        ...
        controller.response.link("first", controller.url_for(:overwrite_params => request.params.merge(:page => 1)))
        controller.response.link("prev", controller.url_for(:overwrite_params => request.params.merge(:page => page - 1))) if page > 1
        controller.response.link("next", controller.url_for(:overwrite_params => request.params.merge(:page => page + 1))) unless total_pages && page < total_pages
        controller.response.link("last", controller.url_for(:overwrite_params => request.params.merge(:page => total_pages))) if total_pages
      end
      super
    end
